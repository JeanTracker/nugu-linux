...
static void *_rec_write_thread(void *data)
{
...
static void *_rec_event_thread(void *data)
{
	qnx_alsa_handle *handle = (qnx_alsa_handle *)data;
	...
	while (1) {
		...
		int rtn = EOK;
		int pcm_fd = snd_pcm_file_descriptor(handle->pcm_handle,
						     SND_PCM_CHANNEL_PLAYBACK);
		       ...
		       NuguAudioProperty prop)
{
	...
	pthread_create(&alsa_handle->write_thread, NULL, _rec_write_thread,
		       alsa_handle);
	pthread_create(&alsa_handle->event_thread, NULL, _rec_event_thread,
		       alsa_handle);

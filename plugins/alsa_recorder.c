...
static void *_rec_read_thread(void *data)
{
	...
	while (1) {
		if (handle->stop) {
			nugu_info("request to stop thread\n");
			break;
		}
		...
		if (snd_pcm_plugin_status(handle->pcm_handle, &status) < 0) {
		...
		if (status.status == SND_PCM_STATUS_CHANGE ||
			...
			if (snd_pcm_plugin_prepare(handle->pcm_handle,
						   SND_PCM_CHANNEL_CAPTURE) <
			    0) {
				fprintf(stderr,
